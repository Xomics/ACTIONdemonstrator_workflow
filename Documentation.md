...
| `SNF_ANALYSIS`   |  | The sample correlation matrix produced by SNF is analyzed using spectral clustering. Then, clusters are analyzed for possible differences in phenotype covariates and behavioral dimensions (MCA)  | `snf.csv`,   | `snf_analysis_out.ipynb` |
...
| `MOFA_ANALYSIS`  |  | Downstream analysis of MOFA. The latent factors in the omics data are linked to phenotypic and behavioral data | `MOFAmodel.hdf5`, Phenotype covariates, `cbcl_mca_coord.csv`, `epigenomics_annotation_out.RData` |  `mofa_analysis.html`, `MOFA_factor_top100_epigenomics_weights/factor{1:10}_weights.csv`|
