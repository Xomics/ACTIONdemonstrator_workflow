...
metaData_subset2 <- metaData_subset[, c('Age', 'Aggression_Tscore', 'Sex, 'Familynumber')]
...
#' @return GEE model
geemodel <- function (data, pheno_feature, family) {
...
run_geemodel <- function(data, pheno_feature,family=gaussian) {
...
for (i in seq(1:10)) {
	...
	coeff <- summary(invisible(geemodel(data, pheno_feature, family)))$coefficients
...
```{r}
metaData_subset2$SNF_label_4 <- as.factor(metaData_subset2$SNF_label_4) 
metaData_subset2s$SNF_label_2 <- as.factor(metaData_subset2$SNF_label_2)
```

### Rewrite male/female as 0/1
```{r}
metaData_subset2$Sex[metaData_subset2$Sex == 'Male'] <- 0
metaData_subset2$Sex[metaData_subset2$Sex == 'Female'] <- 1
metaData_subset2$Sex <- as.numeric(metaData_subset2$Sex)
```
...
geemodel_sex <- run_geemodel(metaData_subset2, 'Sex', 'binomial')
```
...
knitr::kable(geemodel_agg, caption="GEE model with Sex as outcome")
```
